...
# 3. Función para Crear PDF (Versión compatible sin errores)
def generar_pdf(tkt):
    pdf = FPDF()
    pdf.add_page()
    ...
    for item in tkt['items']:
        # Eliminamos caracteres especiales para evitar el error del PDF
        desc = item['Descripción'].replace("ó", "o").replace("í", "i").replace("á", "a").replace("é", "e").replace("ú", "u")
...
if st.session_state.ultimo_ticket:
    ...
    # Botón PDF (Ahora sin errores de codificación)
